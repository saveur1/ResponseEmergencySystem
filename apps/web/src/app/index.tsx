...
function Dashboard() {
  ...
  useEffect(() => {
    const handleFetchIncidents = async () => {
      ...
      let resolution = emergencies.filter(
      ...
      let cancellation = emergencies.filter(
        (incident) => incident.status === 'cancelled'
      ).length
      cancellation = Math.round((cancellation / emergencies.length) * 100)
      if (isNaN(cancellation)) cancellation = 0

      const dispatched = emergencies.filter(
    ...
    setInterval(() => handleFetchIncidents(), 2000)
  }, [])
  ...
  const handleFilterByType = async (
    ...
    if (selectedType === 'all') {
      ...
      const filteredIncidents = incidents.filter(
        (incident) =>
          incident?.type?.toLowerCase() === selectedType.toLowerCase()
      )
